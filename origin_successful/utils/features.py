...
class LogMelExtractor():
    ...
    def transform(self, audio):
        ...
        #print(ham_win.shape,"ham_win")
        window_size = self.window_size
        overlap = self.overlap
        #print(self.melW.shape, "self.melW.shape",window_size ,"window_size",overlap,"overlap")
...
def calculate_multi_logmel(audio_path, sample_rate, feature_extractor):
    ...
    return np.stack([l_feature, r_feature], axis=0)#将生成的声音通道放在除去batchsize后的第零维位置
...
def calculate_multi_features(args):
    ...
    if mini_data:
        ...
        hdf5_path = os.path.join(workspace, 'features', 'logmel', subdir,
                                 '{}_hpss_lrad_2018.h5'.format(data_type))
...
def calculate_features(args):
    ...
    if data_type == 'development':
        meta_csv = os.path.join(dataset_dir, subdir,'meta.csv')
        #meta_csv = os.path.join(dataset_dir, subdir, 'evaluation_setup', 'fold1_evaluate.txt')
    ...
    if mini_data:
        ...
        hdf5_path = os.path.join(workspace, 'features', 'logmel', subdir,
                                 '{}_development_train_(2,320,64).h5'.format(data_type))
    ...
    hf.create_dataset(
        name='feature',
        shape=(0,2,seq_len, mel_bins),
        maxshape=(None,2,seq_len, mel_bins),
        dtype=np.float32)
    ...
    for (n, audio_name) in enumerate(audio_names):
        ...
        # Extract feature
        feature = calculate_multi_logmel(audio_path=audio_path,
                                     sample_rate=sample_rate,
                                     feature_extractor=feature_extractor)
        '''(seq_len, mel_bins)'''
        ...
        hf['feature'].resize((n + 1, 2,seq_len, mel_bins))
        hf['feature'][n] = feature
