...
from features_m import LogMelExtractor, calculate_logmel, calculate_three_logmel
import config
...
def plot(feature):
    ...
    fig, axs = plt.subplots(rows_num, cols_num, figsize=(8, 10))

    data = [feature[i] for i in range(3)]
    # for da in data:
    axs[0].matshow(data[0].T, origin='lower', aspect='auto', cmap='jet')
    axs[1].matshow(data[1].T, origin='lower', aspect='auto', cmap='jet')
    axs[2].matshow(data[2].T, origin='lower', aspect='auto', cmap='jet')
    plt.tight_layout()
...
def test():
    import h5py
    h5 = h5py.File(
        '/home/r506/Downloads/dcase2018_task1-master/features/logmel/'
        'TUT-urban-acoustic-scenes-2018-development/development_hpss_lrad.h5',
        'r')
    feature = h5['feature'][1]
    print(feature.shape)
    plot(feature)
